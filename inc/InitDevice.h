//=========================================================
// inc/InitDevice.h: generated by Hardware Configurator
//
// This file will be regenerated when saving a document.
// leave the sections inside the "$[...]" comment tags alone
// or they will be overwritten!
//=========================================================
#ifndef __INIT_DEVICE_H__
#define __INIT_DEVICE_H__

// USER CONSTANTS
// USER PROTOTYPES

// $[Mode Transition Prototypes]
extern void enter_DefaultMode_from_RESET(void);
// [Mode Transition Prototypes]$

// $[Config(Per-Module Mode)Transition Prototypes]
extern void HFXO_enter_DefaultMode_from_RESET(void);
extern void LFXO_enter_DefaultMode_from_RESET(void);
extern void CMU_enter_DefaultMode_from_RESET(void);
extern void ADC0_enter_DefaultMode_from_RESET(void);
extern void ACMP0_enter_DefaultMode_from_RESET(void);
extern void IDAC0_enter_DefaultMode_from_RESET(void);
extern void RTC_enter_DefaultMode_from_RESET(void);
extern void USART0_enter_DefaultMode_from_RESET(void);
extern void USART1_enter_DefaultMode_from_RESET(void);
extern void LEUART0_enter_DefaultMode_from_RESET(void);
extern void VCMP_enter_DefaultMode_from_RESET(void);
extern void WDOG_enter_DefaultMode_from_RESET(void);
extern void I2C0_enter_DefaultMode_from_RESET(void);
extern void TIMER0_enter_DefaultMode_from_RESET(void);
extern void TIMER1_enter_DefaultMode_from_RESET(void);
extern void PCNT0_enter_DefaultMode_from_RESET(void);
extern void PRS_enter_DefaultMode_from_RESET(void);
extern void PORTIO_enter_DefaultMode_from_RESET(void);
// [Config(Per-Module Mode)Transition Prototypes]$

// $[User-defined pin name abstraction]



#define DSP_CLK_PIN                         (12)
#define DSP_CLK_PORT                        (gpioPortE)

#define DSP_CS_PIN                          (13)
#define DSP_CS_PORT                         (gpioPortE)

#define DSP_DREQ_PIN                        (4)
#define DSP_DREQ_PORT                       (gpioPortF)

#define DSP_SI_PIN                          (10)
#define DSP_SI_PORT                         (gpioPortE)

#define DSP_SO_PIN                          (11)
#define DSP_SO_PORT                         (gpioPortE)

#define DSP_XRESET_PIN                      (3)
#define DSP_XRESET_PORT                     (gpioPortF)

//#define HIGH_FREQUENCY_CLOCK__XTAL_PIN      (13)
//#define HIGH_FREQUENCY_CLOCK__XTAR_PIN      (14)
//#define HIGH_FREQUENCY_CLOCK__XTAL_PORT     (gpioPortB)

//#define LOW_FREQUENCY_CLOCK__XTAL_PIN       (7)
//#define LOW_FREQUENCY_CLOCK__XTAR_PIN       (8)
//#define LOW_FREQUENCY_CLOCK__XTAL_PORT      (gpioPortB)

#define OK_KEYINCOMING_CALL_PIN             (8)
#define OK_KEYINCOMING_CALL_PORT            (gpioPortA)

#define PWM_LED_BLUE_PIN                    (2)
#define PWM_LED_BLUE_PORT                   (gpioPortA)

#define PWM_LED_GREEN_PIN                   (0)
#define PWM_LED_GREEN_PORT                  (gpioPortA)

#define PWM_LED_RED_PIN                     (1)
#define PWM_LED_RED_PORT                    (gpioPortA)

#define TEST_POINT_1_PIN                    (9)
#define TEST_POINT_1_PORT                   (gpioPortA)

#define TEST_POINT_2_PIN                    (10)
#define TEST_POINT_2_PORT                   (gpioPortA)

#define TEST_POINT_3_PIN                    (11)
#define TEST_POINT_3_PORT                   (gpioPortB)

#define TEST_POINT_4_PIN                    (10)
#define TEST_POINT_4_PORT                   (gpioPortC)

#define TIMER0_CC0_PIN                      (0)
#define TIMER0_CC0_PORT                     (gpioPortA)

#define TIMER0_CC1_PIN                      (1)
#define TIMER0_CC1_PORT                     (gpioPortA)

#define TIMER0_CC2_PIN                      (2)
#define TIMER0_CC2_PORT                     (gpioPortA)

#define USART0_CLK_PIN                      (12)
#define USART0_CLK_PORT                     (gpioPortE)

#define USART0_CS_PIN                       (13)
#define USART0_CS_PORT                      (gpioPortE)

#define USART0_RX_PIN                       (11)
#define USART0_RX_PORT                      (gpioPortE)

#define USART0_TX_PIN                       (10)
#define USART0_TX_PORT                      (gpioPortE)

#define USART1_RX_PIN                       (1)
#define USART1_RX_PORT                      (gpioPortC)

#define USART1_TX_PIN                       (0)
#define USART1_TX_PORT                      (gpioPortC)

#define USB_DM_PIN                          (14)
#define USB_DM_PORT                         (gpioPortC)

#define USB_DP_PIN                          (15)
#define USB_DP_PORT                         (gpioPortC)

#define USB_D_PIN                           (14)
#define USB_D_PIN                           (15)

#define USB_D_PORT                          (gpioPortC)
#define USB_D_PORT                          (gpioPortC)

#define USB_ID_PIN_PIN                      (2)
#define USB_ID_PIN_PORT                     (gpioPortF)

#define VBAT_IN_PIN                         (4)
#define VBAT_IN_PORT                        (gpioPortD)

// [User-defined pin name abstraction]$

#endif

