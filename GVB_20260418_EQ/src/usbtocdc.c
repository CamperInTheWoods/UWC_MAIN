...
#define USB_PORT_TX_DMA_CHANNEL   ( 0 )
#define USB_PORT_RX_DMA_CHANNEL   ( 1 )
#define CDC_TX_DMA_SIGNAL         DMAREQ_USART1_TXBL
#define CDC_RX_DMA_SIGNAL         DMAREQ_USART1_RXDATAV

#define USB_PORT                  USART0
#define USB_PORT_CLOCK            cmuClock_USART2
...
STATIC_UBUF(usbTxBuffer0, CDC_USB_TX_BUF_SIZ);   /* UART receive buffers.  */
STATIC_UBUF(usbTxBuffer1, CDC_USB_TX_BUF_SIZ);

static const uint8_t  *usbRxBuffer[  2 ] = { usbRxBuffer0, usbRxBuffer1 };
static const uint8_t  *usbTxBuffer[ 2 ] = { usbTxBuffer0, usbTxBuffer1 };
                           ...
                           USBD_State_TypeDef newState)
{
  if (newState == USBD_STATE_CONFIGURED)
  {
    ...
    usbTxActive    = false;
    dmaRxCompleted = true;
    DMA_ActivateBasic(USB_PORT_RX_DMA_CHANNEL, true, false,
                      (void *) usbTxBuffer[ usbTxIndex ],
                      (void *) &(USB_PORT->RXDATA),
 ...
 //   USBTIMER_Start(CDC_TIMER_ID, CDC_RX_TIMEOUT, UartRxTimeout);
           ...
           (newState != USBD_STATE_SUSPENDED))
  {
    ...
    /* Stop DMA channels. */
    DMA->CHENC = ( 1 << USB_PORT_TX_DMA_CHANNEL ) |
                 ( 1 << USB_PORT_RX_DMA_CHANNEL );
  }
                           ...
                           uint32_t remaining)
{
  ...
  if ((status == USB_STATUS_OK) && (xferred > 0))
  {
    ...
    if (!dmaTxActive)
    {
      ...
      dmaTxActive = true;
      DMA_ActivateBasic(USB_PORT_TX_DMA_CHANNEL, true, false,
                        (void *) &(USB_PORT->TXDATA),
                              ...
                              uint32_t remaining)
{
  ...
  if (status == USB_STATUS_OK)
  {
    if (!dmaRxActive)
    {
      ...
      USBD_Write(CDC_EP_DATA_IN, (void*) usbTxBuffer[ usbTxIndex ^ 1],
                 usbTxCount, UsbDataTransmitted);
      LastUsbTxCnt = usbTxCount;
...
static void DmaRxComplete(unsigned int channel, bool primary, void *user)
{
  ...
  if (dmaRxCompleted)
  {
    usbTxCount = CDC_USB_TX_BUF_SIZ;
  }
  else
  {
    usbTxCount = CDC_USB_TX_BUF_SIZ - 1 -
...
static void UartRxTimeout(void)
{
  ...
  if ((cnt == 0) && (LastUsbTxCnt == CDC_BULK_EP_SIZE))
  {
     ...
     */
    /* Stop Rx DMA channel. */
    DMA->CHENC     = 1 << USB_PORT_RX_DMA_CHANNEL;
    dmaRxCompleted = false;
    /* Call DMA completion callback. */
    DmaRxComplete(USB_PORT_RX_DMA_CHANNEL, true, NULL);
    return;
  }

  if ((cnt > 0) && (cnt == usbTxCount))
  {
    /*
                              ...
                              uint32_t remaining)
{
  ...
  if ((status == USB_STATUS_OK) && (xferred == 7))
  {
    ...
    USB_PORT->FRAME = frame;
    USART_BaudrateAsyncSet(USB_PORT, 0, cdcLineCoding.dwDTERate, usartOVS16);
